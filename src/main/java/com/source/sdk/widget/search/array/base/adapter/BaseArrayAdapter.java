package com.source.sdk.widget.search.array.base.adapter;
...
import com.source.sdk.widget.search.array.base.model.BaseTabModel;
...
public abstract class BaseArrayAdapter<T extends BaseTabModel,H extends BaseArrayHolder<T>> implements IArray {
    ...
    @Override
    public View getView(ViewGroup parentView, View view, int position) {
        H holder;
        if(view == null){
            holder = onCreateViewHolder(parentView,getItemType(position));
            view = holder.getContentView();
            view.setTag(R.id.spearch_array_item_id,holder);
        }else{
            holder = (H) view.getTag(R.id.spearch_array_item_id);
        }
